'''
监控代码：监控服务器的内存、CPU、网络、磁盘等，与租车系统部署到一块
'''
...
print(psutil.disk_usage("e:/").percent)   # 租车系统所在的磁盘百分比
...
# 达到类似server agent的效果，在性能测试期间，获取CPU、内存的趋势
# 死循环，每隔3秒读一次，把读的结果写到文件中，测试结束后分析文件，使用Excel等生成图表
# 时间戳 CPU%  内存%  磁盘%  发送字节数   接收字节数
with open("d:/资源占用情况.txt", mode='a', encoding='utf8') as file:
